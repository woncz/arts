...
class Solution2 implements ISolution {
    @Override
    public int reversePairs(int[] nums) {
        TreeMap<Integer, TreeSet<Integer>> tm = new TreeMap(Comparator.reverseOrder());
        for (int i = 0; i < nums.length; i++) {
            if (tm.containsKey(nums[i])) {
                TreeSet<Integer> tmm = tm.get(nums[i]);
                tmm.add(i);
                tm.put(nums[i], tmm);
            } else {
                TreeSet<Integer> tmm = new TreeSet<>();
                tmm.add(i);
                tm.put(nums[i], tmm);
            }
        }
        int cnt = 0;
        for (int i = 0; i < nums.length; i++) {
            cnt += count(tm, i, nums[i]);
        }
        return cnt;
    }

    int count(TreeMap<Integer, TreeSet<Integer>> tm, int i, int iv) {
        int cnt = 0;
        int ivv = iv / 2;
        boolean include = (ivv * 2 != iv);
        Map<Integer, TreeSet<Integer>> m = tm.tailMap(ivv, include);
        for (Map.Entry<Integer, TreeSet<Integer>> e : m.entrySet()) {
            TreeSet<Integer> ttm = e.getValue();
            cnt += ttm.tailSet(i, false).size();
        }
        return cnt;
    }
}
